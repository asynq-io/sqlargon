...
from sqlargon.orm import Base, Model
from sqlargon.query_builder import Option, UnsupportedOption
from sqlargon.registry import get_default_database
from sqlargon.routing import RoutingContext, RoutingOptions
...
class SQLAlchemyRepository(Generic[Model]):
    ...
    def _get_default_set(cls) -> set[str]:
        # a conflicting row was created when it was created, so ``created_at``
        # keeps the value it was inserted with -- rewriting it would also make
        # ``is_new`` report an upserted row as freshly inserted
        untouched = cls._get_default_index_elements()
        if issubclass(cls.model, CreatedUpdatedMixin):
            untouched = untouched | {"created_at"}
        return {
            c.name
            for c in cls.model.__table__.columns  # type: ignore[attr-defined]
            if c.name not in untouched
        }
